...
To get one genome size estimation per sample instead of one per query, first run the main function with `return_posterior = TRUE` to keep the full posterior distribution of each query:
...
Each sample gets a mean genome size, weighted by the abundance of its queries, and a confidence interval that accounts for the uncertainty of every query:
