...
With the Bayesian method, the full posterior predictive distribution of each query can also be returned, and aggregated into one mean genome size estimate per sample (e.g. from an ASV abundance table), weighted by query abundance, with an interval for that sample mean that accounts for the estimation uncertainty in the genome size of every query.
