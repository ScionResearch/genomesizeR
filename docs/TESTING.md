...
Prerequisites - Install `devtools` and `testthat`:
...
Run the unit tests of the per-sample aggregation (no reference data
needed)
...
Notes - All E2E tests require the reference archive
`genomesizeRdata_v*.tar.gz` in `inst/extdata/`.
